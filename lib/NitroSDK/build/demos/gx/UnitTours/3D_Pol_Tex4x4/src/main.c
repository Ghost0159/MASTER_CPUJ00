/*---------------------------------------------------------------------------*
  Project:  NitroSDK - GX - demos - UnitTours/3D_Pol_Tex4_Plett
  File:     main.c
  ...
  $Log: main.c,v $
  Revision 1.36  2006/01/18 02:11:23  kitase_hirotake
  do-indent

  Revision 1.35  2005/11/21 10:37:38  kitase_hirotake
  SVC_WaitVBlankIntr を OS_WaitVBlankIntr に変更

  Revision 1.34  2005/02/28 05:26:15  yosizaki
  do-indent.

  Revision 1.33  2004/11/02 09:38:30  takano_makoto
  fix typo.

  Revision 1.32  2004/05/24 10:34:55  takano_makoto
  autotestコードの修正
...
//---------------------------------------------------------------------------
// A sample that use a compressed 4x4 texel texture:
//
// Note that the address of texture indices is decided by that of a corresponding texture image.
//
// HOWTO:
// 1. Load texture images and texture indices to the texture image slots by
...
// 3. Specify the texture to use and etc. by G3_TexImageParam(GX_TEXFMT_COMP4x4, ...).
// 4. Specify the palette to use by G3_TexPlttBase(..., GX_TEXFMT_COMP4x4).
...
void NitroMain(void)
{
    ...
    u32     myTexIdxAddr = 0x20000 + (myTexAddr >> 1);  // the address of texture indices is fixed according to the texture image address
    u32     myTexPlttAddr = 0x01000;   // a texture palette at 0x1000 of the texture palette slots

    //---------------------------------------------------------------------------
    // Initialization:
    // They set up interrupt, VRAM and BG0 as 3D mode
    //---------------------------------------------------------------------------
    DEMOInitCommon();
    DEMOInitVRAM();
    DEMOInitDisplay3D();

    // Alter VRAM bank configuration because we use texture slot 0-1.
    ...
    {
        GX_LoadTex((void *)&tex_4x4_64x64[0],   // a pointer to the texture data on the main memory(4 bytes aligned)
                   myTexAddr,          // an offset address in the texture image slots
                   1024                // the size of the texture(s)(in bytes)
            );
        GX_LoadTex((void *)&index_4x4index[0],  // a pointer to the texture index data on the main memory(4 bytes aligned)
                   myTexIdxAddr,       // an offset address in the texture image slots
                   512                 // the size of the indices(in bytes)
            );
    }
    ...
    {
        GX_LoadTexPltt((void *)&pal_4x4plett[0],        // a pointer to the texture data on the main memory(4 bytes aligned)
                       myTexPlttAddr,  // an offset address in the texture palette slots
                       243 * 2 * 2);   // the size of the texture palette(s)(in bytes)
    }
    ...
    while (1)
    {
        ...
        G3_TexImageParam(GX_TEXFMT_COMP4x4,     // use compressed 4x4 texel texture
        ...
        G3_TexPlttBase(myTexPlttAddr,  // the offset of the texture palette
                       GX_TEXFMT_COMP4x4        // compressed 4x4 texel texture
            );
        ...
        EXT_TestScreenShot(100, 0xD4651E9D);
        EXT_TestTickCounter();
#endif //SDK_AUTOTEST
