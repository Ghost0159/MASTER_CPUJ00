/*---------------------------------------------------------------------------*
  ...
  Revision 1.7  2005/11/21 10:31:18  kitase_hirotake
  ...
  Revision 1.4  2004/05/27 09:23:04  takano_makoto
  Add Autotest code.
  ...
  Revision 1.1  2004/03/01 09:01:11  takano_makoto
  Initial check in.
...
//---------------------------------------------------------------------------
// A sample that use OBJ in 1D mapped bitmap mode.
// 
// This sample displays a object on the display in 1D bitmap format.
// 
// USAGE:
...
//  説明：
//　　　　１次元マッピングのビットマップデータを持つＯＢＪを、アフィン変換（回
//      転拡大縮小）を伴って表示する。
//  操作：
//        UP,DOWN    : 表示位置を操作
//        BUTTON_A,B : 拡縮
...
void NitroMain()
{
    ...
    GX_SetOBJVRamModeBmp(GX_OBJVRAMMODE_BMP_1D_128K);   // １次元マッピング

    /* キャラクターデーターの読み込み */
    GX_LoadOBJ((const void *)IMAGE_DATA, 0, IMAGE_DATA_SIZE);
    ...
    while (1)
    {
        ...
        /* ＯＡＭの設定 */
        G2_SetOBJAttr(&s_OAMBuffer[0], // ＯＡＭの場所を指定
                      pos_x,           // Ｘ位置
                      pos_y,           // Ｙ位置
                      0,               // 優先順位
        ...
        EXT_TestScreenShot(100, 0xC8135726);
        EXT_TestTickCounter();
#endif //SDK_AUTOTEST
