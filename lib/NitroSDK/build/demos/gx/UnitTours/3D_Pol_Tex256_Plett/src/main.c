/*---------------------------------------------------------------------------*
  ...
  Revision 1.34  2005/11/21 10:37:14  kitase_hirotake
  SVC_WaitVBlankIntr を OS_WaitVBlankIntr に変更

  Revision 1.33  2005/02/28 05:26:15  yosizaki
  do-indent.

  Revision 1.32  2004/11/02 09:38:12  takano_makoto
  fix typo.

  Revision 1.31  2004/05/24 10:34:24  takano_makoto
  autotestコードの修正
...
//---------------------------------------------------------------------------
// A sample that use a texture with 256 colors texture palette:
//
// HOWTO:
...
// 3. Specify the texture to use and etc. by G3_TexImageParam(GX_TEXFMT_PLTT256, ...).
// 4. Specify the palette to use by G3_TexPlttBase(..., GX_TEXFMT_PLTT256).
...
void NitroMain(void)
{
    ...
    {
        GX_LoadTex((void *)&tex_256plett64x64[0],       // a pointer to the texture data on the main memory(4 bytes aligned)
                   myTexAddr,          // an offset address in the texture image slots
                   4096                // the size of the texture(s)(in bytes)
            );
    }
    ...
    {
        GX_LoadTexPltt((void *)&pal_256plett[0],        // a pointer to the texture data on the main memory(4 bytes aligned)
                       myTexPlttAddr,  // an offset address in the texture palette slots
                       512);           // the size of the texture palette(s)(in bytes)
    }
    ...
    while (1)
    {
        ...
        G3_TexImageParam(GX_TEXFMT_PLTT256,     // use 256 colors palette texture
        ...
        G3_TexPlttBase(myTexPlttAddr,  // the offset of the texture palette
                       GX_TEXFMT_PLTT256        // 256 colors palette texture
            );
        ...
        EXT_TestScreenShot(100, 0x4638E9A1);
        EXT_TestTickCounter();
#endif //SDK_AUTOTEST
