/*---------------------------------------------------------------------------*
  Project:  NitroSDK - ENV - demos - env-1
  File:     main.c
  ...
  $Log: main.c,v $
  Revision 1.6  2006/12/18 07:57:28  yada
  just modify OS_Printf() string.

  Revision 1.5  2006/04/24 00:06:29  okubata_ryoma
  CW2.0ÇÃåxçêÉåÉxÉãã≠âªÇ…î∫Ç¡ÇΩïœçX

  Revision 1.4  2006/01/18 02:11:21  kitase_hirotake
  do-indent

  Revision 1.3  2005/10/14 00:30:48  kitase_hirotake
  fix ENV_Init

  Revision 1.2  2005/09/06 06:30:17  yada
  add comment

  Revision 1.1  2005/09/05 01:25:26  yada
  initial release
...
//================================================================================
//---- resource set 1
ENVResource myResources1[] = {
    "testClass.member", ENV_U32(777),
...
ENVResource *resourceArray[] = {
    myResources1, myResources2, NULL
};
...
void NitroMain(void)
{
    ...
    ENV_Init();
    ENV_AppendResourceSet(myResources2);
    ...
    OS_Printf("==== search resources whose class is 'test'\n");
    {
        ...
        while ((res = ENV_SearchByClass(&iter, "test")) != 0)
        {
            OS_Printf("found [%s]\n", res->name);
        }
    }
    ...
    OS_Printf("\n==== search resources whose member is 'member1'\n");
    {
        ...
        while ((res = ENV_SearchByMember(&iter, "member1")) != 0)
        {
            OS_Printf("found [%s]\n", res->name);
        }
    }
    ...
    OS_Printf("\n==== search resources whose type is u32\n");
    {
        ...
        while ((res = ENV_SearchByType(&iter, ENV_RESTYPE_U32)) != 0)
        {
            OS_Printf("found [%s]\n", res->name);
        }
    }
    ...
    OS_Printf("\n==== search resources whose name contains 'member'\n");
    {
        ...
        while ((res = ENV_SearchByPartialName(&iter, "member")) != 0)
        {
            OS_Printf("found [%s]\n", res->name);
        }
    }
