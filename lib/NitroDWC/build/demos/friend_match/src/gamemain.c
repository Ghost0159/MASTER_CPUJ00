    ...
    while (1){
        ...
        if (stKeyCnt.trg & PAD_BUTTON_A){
            ...
            case GAME_MODE_LOGIN:  // ログイン
                ...
                // 認証用関数を使ってログイン
                //DWC_LoginAsync(stUserID, stPassword, LoginCallback, &returnSeq);
                DWC_LoginAsync(L"なまえ", NULL, LoginCallback, &returnSeq);
                break;
            default:
                break;
            }
...
#endif
#if 0
    // 友達情報の削除・前詰めに合わせて、それに対応するデータも前に詰める
