...
static s32 s_recruitingUserId = DWC_LOBBY_INVALID_USER_ID;  // ミニゲーム募集中のユーザID
static s32 s_hostUserId = DWC_LOBBY_INVALID_USER_ID;        // 参加しに行くミニゲームのホストのユーザID
...
update_login_lobby( void )
{
    ...
    switch(PPW_LobbyProcess())  // ロビー処理更新
    {
...
{
    DWC_ProcessFriendsMatch();  // DWC通信処理更新
    if(PPW_LobbyProcess() != PPW_LOBBY_STATE_READY) // ロビー処理更新
    {
        OS_TPrintf("unexpected lobby state\n");
        return state_disconnect;
    }
    ...
    {
        char filter[128];
        // ミニゲーム募集を受信したので参加者としてマッチメイクを開始する。
        s_hostUserId = s_recruitingUserId;
        
        // マッチメイク時に使うキーをセット。マッチメイクの度にセットする必要があります。
        DWC_AddMatchKeyString(0, s_keys[0].key, s_keys[0].value);
        
        // 募集者がセットしたマッチング指標キーをfilterに指定します。
...
update_matching( void )
{
    ...
    // 一定時間募集者と接続できない時間が続いたらキャンセルする
    if(!PPW_IsConnectedUserId(&s_FMCtrl, s_hostUserId))
    {
        if(timeCount++ > 60 * 60)   // 仮に60秒
        {
            // キャンセル
            timeout = TRUE;
        }
    ...
    {
        // Bボタンもしくはタイムアウトもしくは募集中止されたらでマッチメイクをキャンセルする
        
        OS_TPrintf("Cancel start.\n");
        lobby_stop_recruit();   // 募集者でなくても呼べる
        s_recruitingUserId = DWC_LOBBY_INVALID_USER_ID;
        DWC_CancelMatching();
    }
...
update_matched( void )
{
    ...
    {
        // Bボタンもしくは募集者が切断したら通信を切断し、オンライン状態に戻る。
        // 募集者が抜けた場合も続行してもよい。
