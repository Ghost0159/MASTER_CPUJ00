...
/*---------------------------------------------------------------------------*
  Name:         DTUD_GetAbailableFriendListIndex

  Description:  友達リストの空きインデックスを取得する。

  Arguments:    NONE.

  Returns:      友達リストの空きインデックス
 *---------------------------------------------------------------------------*/
int DTUD_GetAvailableFriendListIndex( void )
{
    int i;

    for ( i = 0; i < FRIEND_LIST_LEN; i++ ){
        if ( !DWC_IsValidFriendData( &s_PlayerInfo.friendList[i] ) )
            return i;
    }
...
void DTUD_DispFriendList( void )
{
    ...
    for ( i = 0; i < FRIEND_LIST_LEN; i++ )
    {
        ...
        DWC_ReportFriendData( &s_PlayerInfo.userData, &s_PlayerInfo.friendList[i] );
    }
    OS_TPrintf( "\n");
}
