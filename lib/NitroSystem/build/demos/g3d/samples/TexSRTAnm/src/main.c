/*---------------------------------------------------------------------------*
  Project:  NITRO-System - demos - g3d - samples - TexSRTAnm
  File:     main.c
...
//---------------------------------------------------------------------------
//
// テクスチャSRTアニメーション再生サンプル
//
// テクスチャSRTアニメーションを再生するには、g3dcvtrによって.itaファイルから
// コンバートされた.nsbtaファイルを使用します。
//
// サンプルの動作
// テクスチャSRTアニメーションが繰り返し再生されます。
// 画面上に 3つの四角ポリゴンが現れます。
// それぞれ 左 … 平行移動
//          中 … 回転
//          右 … スケール 
// のテクスチャ行列アニメーションをしています。
//
// HOWTO:
// 1: .nsbtaファイルをメインメモリにロードします。
...
//    別のNNSG3dAnmObjを確保しなくてはいけません(.nsbtaファイル内のデータは
//    共有できます)。
//
//---------------------------------------------------------------------------
