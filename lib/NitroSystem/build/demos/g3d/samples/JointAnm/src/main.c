/*---------------------------------------------------------------------------*
  Project:  NITRO-System - demos - g3d - samples - JointAnm
  File:     main.c
...
//---------------------------------------------------------------------------
//
// ジョイントアニメーションを再生するサンプル
//
// ジョイントアニメーションを再生するには、g3dcvtrによって.icaファイルから
// コンバートされた.nsbcaを使用します。
//
// サンプルの動作
// ジョイントアニメーションが繰り返し再生されます。
//
// HOWTO:
// 1: .nsbcaファイルをメインメモリにロードします。
// 2: NNS_G3dGetAnmByIdx, NNS_G3dGetAnmByName関数等を用いて再生したい
//    ジョイントアニメーションへのポインタを取得します。
...
//    別のNNSG3dAnmObjを確保しなくてはいけません(.nsbcaファイル内のデータは
//    共有できます)。
// 
//---------------------------------------------------------------------------
...
NitroMain(void)
{
    ...
    G3DDemo_InitConsole();
    G3DDemo_Print(0,0, G3DDEMO_COLOR_YELLOW, "JointAnm");
