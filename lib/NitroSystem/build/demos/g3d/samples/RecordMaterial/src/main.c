/*---------------------------------------------------------------------------*
  Project:  NITRO-System - demos - g3d - samples - RecordMaterial
  File:     main.c
...
//---------------------------------------------------------------------------
//
// マテリアルの計算結果を前計算しておくサンプル
//
// マテリアルの計算結果をバッファしておくことで、マテリアルが不変の場合にCPUの
// 計算コストを節約することができます。
//
// サンプルの動作
// ジョイントアニメーションが繰り返し再生されます。
// Aボタンを押すとバッファされたマテリアル計算結果を使用するか、
// 描画毎に計算しなおすかが切り替わります。
//
// HOWTO:
// 1: NNS_G3dAllocRecBufferMatでバッファの領域を確保します。
// 2: NNS_G3dRenderObjSetMatAnmBufferでバッファをレンダリングオブジェクトに
//    アタッチします。
...
//    をリセットします。
// 7: 次回のNNS_G3dDrawからマテリアルデータを計算する代わりにバッファのデータを参照して描画します。
// 8: NNS_G3dRenderObjReleaseMatAnmBufferでバッファを取り外すことができます。
//
//---------------------------------------------------------------------------
