...
//---------------------------------------------------------------------------
//
// コールバックを使用してG3Dの描画動作をカスタマイズするサンプル（その４）
//
...
// 
// サンプルの動作
// 人が走っていますが、頭の高さは固定されています。
//
// HOWTO:
// 1: 走る人のレンダリングオブジェクトにコールバック関数を設定します。
// 2: コールバック関数は、NNS_G3D_SBC_NODEDESCのNNS_G3D_SBC_CALLBACK_TIMING_Cで
//    呼び出されるようにします。この場所でコールした場合、ジョイントのSRT情報は
//    計算されてカレント行列に入っています。
// 3: 描画前の設定にNNS_G3dGlbFlushVP関数を使用します。この関数を使用した場合、
//    カレント射影行列にはカメラ行列と射影変換行列が合成されたものが設定されます。
// 4: コールバック関数内で、カレント位置座標行列とカレント方向ベクトル行列を取り出します。
// 5: NNS_G3dGlbFlushVP関数を使用した場合、取り出された行列の4行目は、
//    ワールド座標空間でのtranslationになるので、これを加工します。
// 6: 加工した行列をカレント行列に設定します。
//
//---------------------------------------------------------------------------
...
holdHisHead(NNSG3dRS* rs)
{
    ...
    if (NNS_G3dRSGetCurrentNodeDescID(rs) == jntID)
    {
        ...
        //
        // カレント位置座標行列・カレント方向ベクトル行列を取り出す
        // NULLを入れた場合はデータは取り出されません。
        //
        NNS_G3dGetCurrentMtx(&m, &n);

        //
        // NNS_G3dGlbFlushVPを使用している場合、4行目はワールド座標の
        // Translationになる。
        //
        m._31 = 14 * FX32_ONE;                        // 頭のy座標を固定してしまう。

        NNS_G3dGeMtxMode(GX_MTXMODE_POSITION_VECTOR);
        NNS_G3dGeLoadMtx43((MtxFx43*)&n);             // 方向行列なので4行目にゴミが入っても無問題
...
NitroMain(void)
{
    ...
    //
    {
        ...
        SDK_ASSERTMSG( pFile, "anm load failed" );
        {
            ...
            //
            // コールバックを設定する
            // ジョイントのSRT情報がジオメトリエンジンへ転送される直前に呼び出される
            // ようにしておく
            //
