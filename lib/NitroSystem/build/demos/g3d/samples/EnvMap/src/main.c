/*---------------------------------------------------------------------------*
  Project:  NITRO-System - demos - g3d - samples - EnvMap
  File:     main.c
...
//---------------------------------------------------------------------------
// 環境マップされたオブジェクトを表示するサンプル
//
// .imdファイルの<material>要素のtex_gen_modeがnrmに設定されている場合、
// 環境マップを使用してテクスチャをマッピングします。
//
// サンプルの動作
...
/*-------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
static void
rotEnvMap(NNSG3dRS* rs)
{
    //
    // NNS_G3D_SBC_ENVMAP命令にコールバック関数を入れた場合、
    // 行列モードは、GX_MTXMODE_TEXTUREになっていることに注意してください。
    // コールバック関数終了時にも、行列モードがGX_MTXMODE_TEXTUREになっている
    // 必要があります。
    //
    static MtxFx33 mRot = {
        FX32_ONE, 0, 0,
...
NitroMain(void)
{
    ...
    model = LoadG3DModel("data/sphere_env.nsbmd");
    ...
    G3DDemo_InitConsole();
    G3DDemo_Print(0,0, G3DDEMO_COLOR_YELLOW, "EnvMap");
