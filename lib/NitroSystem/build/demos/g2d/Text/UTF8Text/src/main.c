/*---------------------------------------------------------------------------*
  Project:  NITRO-System - demos - g2d - Text - UTF8Text
  File:     main.c
...
//      このデモでは NNS_G2dFontInitUTF8 を使う場合を示しています。
//        TXT_LoadFont( &gFont, TXT_UTF8_FONTRESOURCE_NAME );

        void* pFontFile;
        u32 size;

        size = TXT_LoadFile( &pFontFile, TXT_UTF8_FONTRESOURCE_NAME );
        NNS_G2D_ASSERT( size > 0 );

        NNS_G2dFontInitUTF8(&gFont, pFontFile);
...
static void SampleMain(void)
{
    ...
    NNS_G2dTextCanvasDrawText(&gTextCanvas, 0, 0,
        ...
        "\xE3\x81\x82\xE3\x81\x84\xE3\x81\x86\xE3\x81\x88\xE3\x81\x8A\xE6\xBC\xA2\xE5\xAD\x97"
                                                                    // <- 日本語文字
    );
}
